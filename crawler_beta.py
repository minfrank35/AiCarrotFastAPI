...
def save_to_text_file(article_details, filename='article_details.txt'):
    with open(filename, 'a', encoding='utf-8') as file:
        file.write(f"Article Title: {article_details['title']}\n")
        file.write(f"Price: {article_details['price']}\n")
        file.write(f"Description: {article_details['description']}\n")
        file.write(f"Images: {', '.join(article_details['images'])}\n")
        file.write(f"Nickname: {article_details['nickname']}\n")
        file.write(f"Region: {article_details['region']}\n")
        file.write(f"Temperature: {article_details['temperature']}\n")
        file.write(f"Interest Count: {article_details['interest_count']}\n")
        file.write(f"View Count: {article_details['view_count']}\n")
        file.write("-" * 40 + "\n")
    ...
    if main_page_soup:
        ...
        for link in article_links:
            ...
            if article_details:
                ...
                print(f"Interest Count: {article_details['interest_count']}")
                print(f"View Count: {article_details['view_count']}")
                print("-" * 40)

                # Save the details to a text file
                save_to_text_file(article_details)
